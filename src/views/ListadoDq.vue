<template>
  <v-data-table :headers="headers" :items="desserts" sort-by="calories" class="elevation-1" :search="search">
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>Denuncias y Quejas</v-toolbar-title>
...
export default {
  ...
  methods: {
    async initialize() {
      try {
        const datos = await srv.listado({dptos: this.cnfData.ar1});
        if (datos.ok) {
          console.log("daos", datos)
          this.desserts = datos.body.list;
          this.headers = datos.body.headers;
          this.headers.push({
            text: "Actions",
            value: "actions",
            sortable: false,
