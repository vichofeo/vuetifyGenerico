<template>
    <v-hover v-slot="{ hover }" open-delay="200">
        <v-card v-bind="$attrs" :class="{ ...classes, 'on-hover': hover }" 
        class="v-card--material pa-3"
            :elevation="hover ? 18 : 1" @click="onClick">
            
            <div class="d-flex grow flex-wrap">
                <v-avatar v-if="avatar && avatarName=='BDPA'" 
                ...
                <v-sheet v-else :class="{ 'pa-7': !$slots.image }" :color="color" :max-height="icon ? 90 : undefined"
                    :width="icon ? 'auto' : '100%'" elevation="6" class="text-start v-card--material__heading mb-n6" dark>
