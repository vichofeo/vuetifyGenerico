<template>
    <div v-if="items && headers">
      ...
      <v-data-table
        ...
        v-if="!expand"
      >
        <template v-slot:item.alertax23="{ item }">
          <ChipAlerta :mensaje="item.alertax23"></ChipAlerta>
        </template>
      ...
      <v-data-table
        ...
        item-key="idx"
        class="elevation-1"
        :item-class="row_classesExpand"
        :page.sync="page"
        :items-per-page="itemsPerPage"
        hide-default-footer
        @page-count="pageCount = $event"
        disable-sort
        @click:row="viewExpand"
        v-else
      >
  ...
  <script>
  import ChipAlerta from "./ChipAlerta.vue";
  export default {
    components: {
      ChipAlerta,
    },
    ...
    computed: {
      ...
      datosHeaders() {
        return this.headers.map((o) => ({
          ...o,
          class: `${this.opColor} darken-2 font-weight-light white--text`,
        }));
      },
    },
  };
